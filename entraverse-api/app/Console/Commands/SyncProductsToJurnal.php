...
class SyncProductsToJurnal extends Command
{
    ...
    public function handle(JurnalProductService $jurnalProduct): int
    {
        $productId = (string) $this->option('id');
        $useQueue = (bool) $this->option('queue');

        if ($productId !== '') {
            return $this->syncSingleProduct($jurnalProduct, $productId, $useQueue);
        }

        return $this->syncAllProducts($jurnalProduct, $useQueue);
    }

    protected function syncSingleProduct(JurnalProductService $jurnalProduct, string $productId, bool $useQueue): int
    {
    ...
    protected function syncAllProducts(JurnalProductService $jurnalProduct, bool $useQueue): int
    {
        ...
        $query->chunk($chunkSize, function ($products) use ($jurnalProduct, $useQueue, &$successCount, &$failedCount, $bar): void {
