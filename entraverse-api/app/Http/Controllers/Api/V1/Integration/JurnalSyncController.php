...
class JurnalSyncController extends Controller
{
    ...
    {
        $configuration = $this->mekariConfigurationSummary();

        if (! $configuration['credentials_configured']) {
            return response()->json([
                'success' => false,
                'message' => 'Jurnal integration is not configured.',
                'connected' => false,
        ...
        try {
            $response = $this->jurnalProduct()->getProducts([
                'page' => 1,
                'per_page' => 1,
            ]);
            ...
            return response()->json([
                'success' => true,
                'message' => 'Jurnal integration is connected.',
                'connected' => true,
                'configuration' => $configuration,
                'data' => [
                    'sample_response_keys' => array_keys($response),
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to connect to Jurnal.',
                'connected' => false,
                'error' => $exception->getMessage(),
                'configuration' => $configuration,
                'data' => $exception->getResponseBody(),
            ...
            return response()->json([
                'success' => false,
                'message' => 'Unexpected error while checking Jurnal integration.',
                'connected' => false,
                'error' => $exception->getMessage(),
                'configuration' => $configuration,
            ], 500);
    ...
    {
        $jurnalProduct = $this->jurnalProduct();
        $results = [
            'success' => [],
            'failed' => [],
        ];
        ...
        foreach ($products as $product) {
            try {
                $jurnalProduct->syncProduct($product);
    ...
    protected function jurnalProduct(): JurnalProductService
    {
        return app(JurnalProductService::class);
    }

    protected function configurationErrorResponse(InvalidArgumentException $exception): JsonResponse
    {
        return response()->json([
            'success' => false,
            'message' => 'Jurnal integration is not configured.',
     ...
     * }
     */
    protected function mekariConfigurationSummary(): array
    {
        $clientId = trim((string) config('services.mekari.client_id', ''));
        $clientSecret = trim((string) config('services.mekari.client_secret', ''));
        $webhookSecret = trim((string) config('services.mekari.webhook_secret', ''));

        return [
            'base_url' => rtrim((string) config('services.mekari.base_url', 'https://api.mekari.com'), '/'),
            'jurnal_base_path' => rtrim((string) config('services.mekari.jurnal_base_path', '/public/jurnal/api/v1'), '/'),
            'credentials_configured' => $clientId !== '' && $clientSecret !== '',
            'webhook_secret_configured' => $webhookSecret !== '',
            'debug_signing' => (bool) config('services.mekari.debug_signing', false),
            'timeout' => (int) config('services.mekari.timeout', 30),
            'connect_timeout' => (int) config('services.mekari.connect_timeout', 10),
        ];
    }
