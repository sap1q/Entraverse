...
it('returns jurnal integration status for authenticated user', function (): void {
    Sanctum::actingAs($this->user);
    ...
    $response = $this->getJson('/api/v1/integrations/jurnal/status');

    $response
        ->assertOk()
        ->assertJsonPath('success', true)
        ->assertJsonPath('connected', true)
        ->assertJsonPath('message', 'Jurnal integration is connected.')
        ->assertJsonPath('data.products_count', 1)
        ->assertJsonPath('configuration.credentials_configured', true);
});
...
it('returns a clean configuration error when jurnal credentials are missing', function (): void {
    Sanctum::actingAs($this->user);
    app()->forgetInstance(JurnalProductService::class);
    config([
        'services.mekari.client_id' => '',
        'services.mekari.client_secret' => '',
    ]);
    ...
    $response
        ...
        ->assertJsonPath('message', 'Jurnal integration is not configured.')
        ->assertJsonPath('configuration.credentials_configured', false);
});
